  ...
  return (
    <ProtectedRoute allowedRoles={["head"]}>
      <div className="p-8 max-w-6xl mx-auto space-y-8">
        <header className="flex justify-between items-center">
          <div>
            ...
            {headProfile?.departmentName ? (
              ...
              <p className="text-slate-500 mt-1">Review submissions and manage department users.</p>
            )}
          </div>
          <button 
        ...
        {inviteLink && (
          <div className="bg-green-50 text-green-800 p-4 rounded-lg flex items-center justify-between border border-green-200">
            <span className="font-mono text-sm max-w-[80%] truncate">{inviteLink}</span>
            <button 
              onClick={() => navigator.clipboard.writeText(inviteLink)}
              className="text-sm bg-white px-3 py-1 rounded shadow-sm hover:bg-slate-50"
            >
              Copy
            </button>
          </div>
